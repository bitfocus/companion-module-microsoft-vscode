...
export function configureServer(password: string, sticky: boolean) {
...
export function startServer(host: string, port: number) {
  ...
  server.on('connection', (con) => {
    ...
    con.on('message', (message) => {
      ...
      if (client) handleMessage(client, message.toString())
    })
  })
}
...
export async function request(data: Request): Promise<Response> {
  ...
  if (!primary) return new Promise<Response>((res, rej) => rej())
